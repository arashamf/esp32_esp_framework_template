...
void wifi_init_sta(void)
{
    ...
    //Register an instance of event handler to the default loop. This function does the same as 
    //esp_event_handler_instance_register_with, except that it registers the handler to the default event loop.
    ...
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA) ); //Set the WiFi operating mode as station, soft-AP, station+soft-AP or NAN. The default mode is station mode.
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config) ); //Set the configuration of the STA, AP or NAN
